...
function SurveyCop({ question, onVote }) {
  ...
  return (
    <div
      className={
        "bg-[#e0f4ff] my-10 p-4 text-2xl rounded-3xl space-y-3 shadow-lg"
      }
    >
      <div className={"flex"}>
        <p>
      ...
      <div
        className={
          "pl-4 w-full h-14 bg-white rounded-2xl mb-4 flex items-center"
        }
        style={{
  ...
  return (
    <>
      {questionState.map((q, index) => {
        return <SurveyCop key={index} question={q} onVote={onVote} />;
      })}
    </>
  );
}
